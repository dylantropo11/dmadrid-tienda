...
public interface ProductoRepository extends JpaRepository<Producto, Integer> {
    ...
    //Consulta ampliada: productos de una categoría, con precio tope OPCIONAL
    @Query(value = "SELECT p FROM Producto p "
            + "WHERE p.categoria.descripcion = :nombreCategoria "
            + "AND (:precioTope IS NULL OR p.precio <= :precioTope) "
            + "ORDER BY p.precio ASC")
    public List<Producto> consultaPorCategoriaYTope(@Param("nombreCategoria") String nombreCategoria,
            @Param("precioTope") Double precioTope);
}
